...
def test_add_event(api):
    ...
    assert (
        len(events_after["data"]["events"])
        - len(events_before["data"]["events"])
        == 1
    )
...
def test_delete_event(api):
    ...
    api.delete_event(
        created_event["data"]["payload"]["id"],
        created_event["data"]["startAt"],
    )
